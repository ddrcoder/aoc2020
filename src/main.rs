...
mod ignore {
        ...
        if !gold {
            passwords
                .map(|(min, max, ch_constrained, password)| {
                    (min..=max)
                        .contains(&password.chars().filter(|ch| *ch == ch_constrained).count())
                })
                .count()
        } else {
            passwords
    ...
    fn sum_groups<
        State,
        MergeLine: Fn(Option<State>, &str) -> State,
        GetCount: Fn(&State) -> usize,
    >(
        lines: Vec<String>,
    ...
    fn day14(gold: bool) -> usize {
        ...
        for line in input {
            if let Some(mask) = line.strip_prefix("mask = ") {
                let (new_or, new_and, new_float) =
                    mask.chars()
                        .rev()
                        .enumerate()
                        .fold((0, !0, 0), |(or, and, float), (b, ch)| match ch {
                            '1' => (or | (1 << b), and, float),
                            '0' => (or, and & !(1 << b), float),
                            'X' => (or, and, float | (1 << b)),
                            _ => (or, and, float),
